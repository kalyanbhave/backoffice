...
namespace SafeNetWS.business.response.writer
{
    ...
    public class SimpleValueResponse
    {
        ...
        public SimpleValueResponse(string inputValue)
        {
            // Initialisation
        ...
        {
            return String.Format("Input value = {0}", "XXXX");
        }
       ...
       /// </summary>
        ...
        {
            string providedData = Util.Nvl(GetInputValue(), string.Empty);
            if (!IsError() && !String.IsNullOrEmpty(providedData))
            {
