...
namespace SafeNetWS.business.response.reader
{
     ...
     * par les méthodes simples de ce service
     * Une méthode simple est une méthode qui retourne un type simple
     * (string, int, date, bool, number,...)
     * L'entrée réponse est structurée de la manière suivante :
     * <?xml version="1.0" encoding="ISO-8859-1"?>
     * <Response>
     *   <Duration>Valeur de retour</Duration>
     *   <Value>Valeur de retour</Value>
     *   <Exception>
     *           <Count>0</Count>
     *           <Message></Message>
     *  </Exception>
     ...
     * 
     */
    public class SimpleResponseReader
    {
        // Tag contenant la valeur
        private const string Xml_Response_Value_TagName = "Value";
        ...
        public SimpleResponseReader(string inputResponse)
        {
        ...
        private void ParseResponse()
        {
            ...
            try
            {
                ...
                if (this.ExceptionCount == 0)
                {
                    ...
                    this.Value = doc.GetElementsByTagName(Xml_Response_Value_TagName)[0].InnerXml;
                    this.Duration = Util.ConvertStringToDouble(doc.GetElementsByTagName(Xml_Response_Duration_TagName)[0].InnerXml);
                }
            }
            catch (Exception e)
            {
                throw new Exception("Erreur lors de la lecture de la réponse simple! Erreur :" + e.Message);
            }
